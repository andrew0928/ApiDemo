...
namespace Andrew.ApiDemo.SDK.Security
{
    public abstract class TokenBase
    {
        ...
        protected TokenBase(string tokenText)
        {
            this.SetEncodeText(tokenText, false);
            //if (this.TypeHash != this.GetTypeHashCode()) throw new FormatException("Token format not correct (type hashcode not match).");
            
            // TODO: prepare for next time uncomment.
            //if (this.CompareBytes(this.TypeFullNameHash, this.GetTypeFullNameHashBytes()) == false) throw new FormatException("Token format not correct (TypeFullNameHash not match).");
        }
        ...
        private string GetEncodeText()
        {
            ...
            return
                HashUtil.BufferToText(buffer) + 
                HASH_SPLIT_CHAR +   //  分段
                HashUtil.BufferToText(HashUtil.ComputeHash(buffer, PUBLIC_HASH_SALT)) +
                HASH_SPLIT_CHAR +   //  分段
                HashUtil.BufferToText(HashUtil.ComputeHash(buffer, PRIVATE_HASH_SALT));
        }
        ...
        private void SetEncodeText(string text, bool validateHash)
        {
            byte[] buffer = null;
            {
                //
                ...
                buffer = HashUtil.TextToBuffer(segments[0]);
                byte[] hash_with_public_salt = HashUtil.TextToBuffer(segments[1]);
                byte[] hash_with_private_salt = HashUtil.TextToBuffer(segments[2]);

                if (HashUtil.CompareHash(buffer, hash_with_public_salt, PUBLIC_HASH_SALT) == false)
                {
                    throw new FormatException("Token hash(public) not validate.");
                }

                this.IsSafe = HashUtil.CompareHash(buffer, hash_with_public_salt, PUBLIC_HASH_SALT);

                this.IsSecure = this.IsSafe && HashUtil.CompareHash(buffer, hash_with_private_salt, PRIVATE_HASH_SALT);
            }
        ...
        public static TToken GetToken<TToken>(string tokenText) where TToken : TokenBase, new()
        {
            return GetToken<TToken>(tokenText, false);
        }

        public static TToken GetToken<TToken>(string tokenText, bool safe) where TToken : TokenBase, new()
        {
            TToken token = new TToken();
            token.SetEncodeText(tokenText, safe);
            return token;
        }

        public static bool IsTokenSafe<TToken>(string tokenText) where TToken : TokenBase, new()
        {
            return GetToken<TToken>(tokenText).IsSafe;
        }

        public static bool IsTokenSecure<TToken>(string tokenText) where TToken : TokenBase, new()
        {
            return GetToken<TToken>(tokenText).IsSecure;
        }

        public static bool IsTokenValidate<TToken>(string tokenText) where TToken : TokenBase, new()
        {
            return GetToken<TToken>(tokenText).IsValidate;
        }
    }
}
